...
cc.Class({
    ...
    onLoad: function() {
        ...
        event.addEvent("event_select", this.otherPlayerDir);
    ...
    fistInit: function() {
        cc.miroGame.userId != cc.miroGame.userList[0].userId && cc.miroGame.that.initMiro();
    },
    initMiro: function() {
        this._player1 = {
            cell: null,
    ...
    miroBegin: function() {
        ...
        this.displayPRoute(this._player1);
        this.setPlayerPos(this._player2, this._aMap[this._iMapRows - 1][0]);
        this.displayPRoute(this._player2, !0);
    ...
    createMap: function() {
        ...
        this._aRightRoute = this.searchRoute(this._aMap[this._iMapRows - 1][0], !0);
        ...
        for (var n = !1, o = 0, s = 0; s < this._aRightRoute.length; s++) {
            for (var a = 0; a < this._aRightRoute[s].around.length; a++)
                0 == this._aRightRoute[s].around[a].cell.state && (
                    ...
                    30 < this.searchRoute(this._aRightRoute[s].around[a].cell, !1, 30, 50).length && o++,
                    1 < o && (n = !0)
                );
            if (n) break;
        }
        ...
        for (var f = 0; f < this._iMapRows; f++)
            for (var m = 0; m < this._iMapCols; m++)
                0 == this._aMap[f][m].state && this.searchRoute(this._aMap[f][m], !1);
    },
    searchRoute: function(_cell, state) {
            ...
            for (var f = cell_arr[cell_arr.length - 1], h = f.around.concat(); ;) {
                if (0 == h.length) {
                    ...
                    if (state) {
                        ...
                        0 < cell_arr.length && (l = cell_arr[cell_arr.length - 1]).route.splice(l.route.indexOf(this.getOppositeDirection(c)), 1);
                    }
                    break;
                }
    ...
    displayPRoute: function(scene_scr) {
                        ...
                        cc.callFunc(
                            function() {
                                if (0 == cell_pos.row && cell_pos.col == self_this._iMapCols - 1) {
                                    ...
                                    self_this.showGameResult(player, sel_player);
                                    ...
                                    self.oppPlayerMoveArray && 0 < self.oppPlayerMoveArray.length ?
                                        ...
                                        :
                                        self_this.displayPRoute(player);
                            }
                        )
                    )
                );
    ...
    onP1Outline: function() {
        cc.miroGame.that.openGameResult(-1);
    },
    onP2Outline: function() {
    ...
    miroTBover: function() {
        if (this.setIntervalID && clearInterval(this.setIntervalID), !cc.miroGame.is_single && !cc.miroGame.b_isAI)
            if (cc.miroGame.userId == cc.miroGame.userList[0].userId) {
                ...
                event.addEvent("event_fistInit", game_that.fistInit);
            }
    },
    openGameResult: function(result_data) {
        var self_that = cc.miroGame.that;
        self_that.showGameResult(self_that._player1, result_data);
    },
    showGameResult: function(player, result_data) {
        var self = this;
        this._isBegin = !1;
    ...
    otherPlayerDir: function(index) {
        var self_that = cc.miroGame.that,
            arrow = [ "", "left", "right", "bottom", "up" ];
        cc.log("otherPlayerDir", index, arrow[index]);
    ...
    onEnable: function() {
        event.addEvent("Enemy_outoLine", this.onP2Outline);
        event.addEvent("Self_outoLine", this.onP1Outline);
        event.addEvent("Self_outoLineShow", this.onGameToBack);
        cc.miroGame.state || this.onP2Outline();
    },
    onDisable: function() {
